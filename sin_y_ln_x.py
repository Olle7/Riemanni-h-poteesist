...
\Pi_{k_2=0}^{M-1}(factorial(floor(k_1/(2*M)^{k_2})\%(2*M))^{-1}*(2*k_2+1)^{-floor(k_1/(2*M)^{k_2})\%(2*M)} )\\
    ...
    for k1 in range((2*M)**M):
        ...
        s1+=p
    return s1
def cos_y_ln_x_multinomial_2(x,y,M=6):
    latex=r"""$\forall_x(x>0 \to sin(y*ln(x))=\\
lim_{M \to \infty}(\sum_{k_1=0}^{(2*M)^M-1}(\\
(\sum_{k_2=0}^{M-1}((floor(k_1/(2*M-1)^{k_2})\%(2*M-1))+1)\%2)*\\
(\sum_{k_2=0}^{M-1}(floor(k_1/(2*M-1)^{k_2})\%(2*M-1))<2*M-1)*\\%sulgudega 0
(y*2)^{\sum_{k_2=0}^{M-1}(floor(k_1/(2*M-1)^{k_2})\%(2*M-1))}*\\
(-1)^{\sum_{k_2=0}^{M-1}(floor(k_1/(2*M-1)^{k_2})\%(2*M-1))/2}*\\
(x-1)^{ \sum_{k_2=0}^{M-1}((2*k_2+1)*(floor(k_1/(2*M-1)^{k_2})\%(2*M-1)))}*\\
(x+1)^{-\sum_{k_2=0}^{M-1}((2*k_2+1)*(floor(k_1/(2*M-1)^{k_2})\%(2*M-1)))}*\\
\Pi_{k_2=0}^{M-1}(factorial(floor(k_1/(2*M-1)^{k_2})\%(2*M-1))^{-1}*(2*k_2+1)^{-floor(k_1/(2*M-1)^{k_2})\%(2*M-1)} )\\
))/2\\%k2-summa ja lim'i lopusulg sel real.
)%forall lopusulg$"""
    s1=0
    for k1 in range((2*M)**M):
        p=1
        ...
        s2=0
        for k2 in range(M):
            s2+=(2*k2+1)*((k1//(2*M-1)**k2)%(2*M-1))
        p*=(x-1)**s2
        p*=(x+1)**-s2

        for k2 in range(M):
            p*=factorial((k1//(2*M-1)**k2)%(2*M-1))**(-1)*(2*k2+1)**(-((k1//(2*M-1)**k2)%(2*M-1)))
        s1+=p
    return s1/2
def cos_y_ln_x_multinomial_4(x,y,M=6):
    latex=r"""$\forall_x(x>0 \to sin(y*ln(x))=\\
lim_{M \to \infty}(\sum_{k_1=0}^{(2*M)^M-1}(\\
(\sum_{k_2=0}^{M-1}((floor(k_1/(2*M-1)^{k_2})\%(2*M-1))+1)\%2)*\\
(y*2)^{\sum_{k_2=0}^{M-1}(floor(k_1/(2*M-1)^{k_2})\%(2*M-1))}*\\
